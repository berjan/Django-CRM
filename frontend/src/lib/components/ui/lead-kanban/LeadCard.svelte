<script>
  import { AlertCircle, Building2 } from '@lucide/svelte';
  import { LEAD_EMAIL_STATUS } from '$lib/constants/lead-email-status.js';
  import LeadEmailStatusBadge from './LeadEmailStatusBadge.svelte';
   ...
   * @property {'inbound'|'outbound'|null} [latest_email_direction]
   * @property {'inbound'|'outbound'|null} [latestEmailDirection]
   * @property {string|null} [last_email_at]
   * @property {string|null} [lastEmailAt]
   * @property {boolean} [email_follow_up_overdue]
   * @property {boolean} [emailFollowUpOverdue]
  ...
  const emailStatus = $derived(item.email_status || item.emailStatus || LEAD_EMAIL_STATUS.NONE);
  ...
  const showGenericOverdue = $derived(isOverdue && emailStatus !== LEAD_EMAIL_STATUS.FOLLOW_UP);
...
>
  ...
  {#if amount || showGenericOverdue || emailStatus !== LEAD_EMAIL_STATUS.NONE || assignees.length > 0}
    <div class="mt-2 flex items-center justify-between gap-2">
      <div class="flex min-w-0 flex-wrap items-center gap-1.5">
        {#if emailStatus !== LEAD_EMAIL_STATUS.NONE}
          <LeadEmailStatusBadge status={emailStatus} draftCount={emailDraftCount} {lastEmailAt} />
        {/if}
        {#if amount}
          <span
